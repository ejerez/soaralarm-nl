...
function displayCacheKey(model, timeStart, timeEnd, selectedWings, weight, customWind, windMin, windMax) {
  return `soar_display_v6:${cacheScope()}:` + JSON.stringify({ model, timeStart, timeEnd, selectedWings, weight, customWind, windMin, windMax, _l: !!_cfg.local })
}
...
function saveDisplayCache(key, display, certainty) {
  try {
    ...
    try {
      Object.keys(localStorage).filter(k => k.startsWith('soar_display_')).forEach(k => localStorage.removeItem(k))
      localStorage.setItem(key, JSON.stringify({ display, certainty, savedAt: Date.now() }))
    } catch {}
  }
}
